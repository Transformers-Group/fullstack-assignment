...
The big challenge here is that we want the response to stream to the user! This means that each token produced by the model is sent to the front-end seperately. To give the user the impression that the data is being generated we require that each token is being displayed as soon as it comes in. 
...
**Hint:** Have a look at the [StreamingResponse](https://fastapi.tiangolo.com/advanced/custom-response/#streamingresponse) object in FastAPI.
...
**Hint**: For reading the stream, we suggest you look at https://canjs.com/doc/can-ndjson-stream.html. But please use `async`/`await` syntax. 

To get a feel of the schema of the API, habe a look at the snippets below.
