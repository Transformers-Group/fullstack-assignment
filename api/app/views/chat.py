...
async def chat(data: ChatRequest):
    chain = _get_agent_executor(
        ...
        system_prompt='You are a helpfull assistant called Astrid that can answer questions about any topic. You are are employed at Transformers group and are tasked with making the employees happy.',
    )
    ...
    async def subscription_wrapper():
        try:
            ...
            yield {
                "type": "error",
                "contents": {"step": "An error occured while responding."},
            }
        finally:
            yield {
                "type": "finish",
    ...
    return StreamingResponse(
        utils.stream_subscription_as_ndjson(subscription_wrapper()),
